// Header file containing hardware configurations for the Arduino Due mapping to the linear actuators.
//
#pragma once

// Arduino value bounds (defined by analog read/write)
#define MIN_POS 0
#define MAX_POS 1023
#define MAX_PWM 255

// Direction pins
// #define DIR_PIN_1 43
// #define DIR_PIN_2 41
// #define DIR_PIN_3 39
// #define DIR_PIN_4 37
// #define DIR_PIN_5 35
// #define DIR_PIN_6 33
#define DIR_PIN_1 35
#define DIR_PIN_2 33
#define DIR_PIN_3 31
#define DIR_PIN_4 41
#define DIR_PIN_5 39
#define DIR_PIN_6 37

// PWM pins
#define PWM_PIN_1 9
#define PWM_PIN_2 10
#define PWM_PIN_3 11
#define PWM_PIN_4 5
#define PWM_PIN_5 6
#define PWM_PIN_6 7

// PA-14P potentiometer pins
#define POT_PIN_1 A11
#define POT_PIN_2 A10
#define POT_PIN_3 A9
#define POT_PIN_4 A8
#define POT_PIN_5 A7
#define POT_PIN_6 A6

// Enable for actuator sets (grouped as such due to different H-bridge controllers)
#define ENABLE_MOTORS_1 24
#define ENABLE_MOTORS_2 25
